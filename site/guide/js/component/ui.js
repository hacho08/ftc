...
const helperBox = {
	...
	open: () => { //도움말 열기
		if (helperArea.length > 0) {  //해당 클래스 존재할떄만 실행
			...
			const $header = document.querySelector("#header .head-body > .wrap");
			...
			if (wrap.classList.contains('flexible')) { // 화면 사이즈 줄어들면 영역도 줄어들게
	...
	close: () => { //도움말 접기
		...
		if (wrap.classList.contains('flexible')) { // 도움말 닫히면 컨텐츠 영역 늘리기
	...
	heightSet: () => {
		...
		const contsAreaH = window.wrapHeight - helperTitH - contsPt;
...
/* ** 스크롤 네비게이션 ** */
const winHeight = window.wrapHeight;
const quickIndicators = document.querySelectorAll('.quick-listA');
...
window.addEventListener("DOMContentLoaded", () => {
	...
	setTimeout(() => { //lnb footer 등 include영역으로 로딩시간이 필요한경우 settimeout에 넣어줌 (배포시 삭제필요)
		/* ** 도움말 ** */
		helperBox.init();
		if (winSize == 'pc') {
			helperBox.open();
